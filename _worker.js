...
// let proxyIP = '8.219.240.160';
// let proxyIP = '199.34.228.173';
let proxyIP = '104.21.7.51';
...
const getNormalConfigs = async (env, hostName, client) => {
    ...
    ports.forEach(port => {
        Addresses.forEach((addr, index) => {
            ...
            vlessWsTls += 'vless' + `://${userID}@${addr}:${port}?encryption=none&type=ws&host=${randomUpperCase(hostName)}${defaultHttpsPorts.includes(port)
                ? `&security=tls&sni=${randomUpperCase(hostName)
                }&fp=randomized&alpn=${client === 'singbox' ? 'http/1.1' : 'h2,http/1.1'
                }`
                : ''}&path=${`/${getRandomPath(16)}${proxyIP ? `/${encodeURIComponent(btoa(proxyIP))}` : ''}`}${client === 'singbox'
